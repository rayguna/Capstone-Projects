...
def download_jcamp_from_nist(df, key, minimum_file_size=1000, no_of_files=20):
    ...
    Args:
    df=a pandas DataFrame consisting a CAS no. Set CAS no. column as the index.
    key=a string input that refers to the chemical name.
    ...
    the saved name has the format cas number_key.jdx
    ...
    for cas_no in df.index: #the index column of the DataFrame is the CAS number
        ...
        try:
            ...
            if len(requests.get(url).content) >= minimum_file_size:
                urllib.request.urlretrieve(url, "reference/%s" %(cas_no+'_'+key+'.jdx'))  # save file according to its cas_no
